...
**NOTE:** the user is responsible to provide a vector `X` of mole fractions of
species that sums up to one. If this is not respected RADCAL fails. The Following
list provides the indexes of available species in vector `X`.
...
Creates a custom database by generating `repeats` samples of `samplesize` rows.
Inputs for `runradcalinput` are to be generated by a `sampler!` user-defined
function which modifies in place a pre-allocated array of compositions, and
returns `T`, `L`, `P`, `FV`, `TWALL` for setting up a simulation. Files are
temporarilly stored under `data/` with a sequential numbered naming during
database creation and finally aggregated in a HDF5 file named after `saveas`.
The choice to aggregate files after an initial dump is because generation can
be interrupted and manually recovered in an easier way and avoiding any risk
of data losses - database creation can take a very long time. If `cleanup`
is `true`, all intermediate files are removed.
"""
